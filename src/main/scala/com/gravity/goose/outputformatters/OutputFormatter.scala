    ...
    if (topNode != null) {
      ...
      for (item <- links) {
        if (item.getElementsByTag("img").isEmpty) {
           ...
           *  __MOD__
           *  
           * Since version 1.11.1, the baseUri is ignored
           * for TextNodes  
           *  
           */
          val tn = new TextNode(item.text) //new TextNode(item.text, baseUri)
          item.replaceWith(tn)
        }
      }
    }
  ...
  private def replaceTagsWithText(topNode: Element) {
    if (topNode != null) {
      ...
      for (item <- bolds) {
         ...
         *  
         */
        val tn = new TextNode(getTagCleanedText(item)) //new TextNode(getTagCleanedText(item), baseUri)
        item.replaceWith(tn)
      }
